...
public class JabRef {
    ...
    public void start(String[] args) {
        ...
        if (prefs.hasKey(JabRefPreferences.DELETE_PLUGINS) && (prefs.get(JabRefPreferences.DELETE_PLUGINS).length() > 0)) {
            String[] toDelete = prefs.getStringArray(JabRefPreferences.DELETE_PLUGINS, null);
            PluginInstaller.deletePluginsOnStartup(toDelete);
            prefs.put(JabRefPreferences.DELETE_PLUGINS, "");
        }

        if (JabRefPreferences.getInstance().isUseProxy()) {
            // NetworkTab.java ensures that proxyHostname and proxyPort are not null
            System.setProperty("http.proxyHost", prefs.getProxyHostname());
            System.setProperty("http.proxyPort", prefs.getProxyPort());
        ...
        Globals.NEWLINE = Globals.prefs.getNewline();
                        ...
                        if (!pr.isInvalid()) {
                            try {
                                ...
                                SaveSession session = FileActions.saveDatabase(pr.getDatabase(),
                                        ...
                                        false, false, JabRefPreferences.getInstance().getDefaultEncoding(), false);
    ...
    private void setLookAndFeel() {
        try {
            ...
            if (Globals.prefs.isUseDefaultLookAndFeel()) {
                // Use system Look & Feel by default
                lookFeel = systemLnF;
            } else {
                lookFeel = JabRefPreferences.getInstance().getLookAndFeel();
            }
            ...
            if (lookFeel.equals("javax.swing.plaf.metal.MetalLookAndFeel")) {
                ...
                try {
                    ...
                    JabRefPreferences.getInstance().setLookAndFeel(systemLnF);
                    // notify the user
        ...
        boolean overrideDefaultFonts = Globals.prefs.isOverrideDefaultFonts();
    ...
    private void openWindow(Vector<ParserResult> loaded) {
        ...
        if (!cli.isBlank() && Globals.prefs.isOpenLastEdited() && (Globals.prefs.get(JabRefPreferences.LAST_EDITED) != null)) {
            // How to handle errors in the databases to open?
            String[] names = Globals.prefs.getStringArray(JabRefPreferences.LAST_EDITED, null);
        ...
        GUIGlobals.CURRENTFONT =
                new Font(Globals.prefs.getFontFamily(), Globals.prefs.getFontStyle(),
                        Globals.prefs.getFontSize());
    ...
    public static ParserResult openBibFile(String name, boolean ignoreAutosave) {
        ...
        try {
            ...
            String encoding = Globals.prefs.getDefaultEncoding();
