...
public class MainTable extends JTable {
	...
	public MainTable(MainTableFormat tableFormat, EventList<BibtexEntry> list, JabRefFrame frame,
		...
		setAutoResizeMode(Globals.prefs.getAutoResizeMode());
		...
		tableColorCodes = Globals.prefs.isTableColorCodesOn();
		selectionModel = new EventSelectionModel<BibtexEntry>(sortedForGrouping);
		setSelectionModel(selectionModel);
		pane = new JScrollPane(this);
		pane.setBorder(BorderFactory.createEmptyBorder());
		pane.getViewport().setBackground(Globals.prefs.getColor(null, JabRefPreferences.getInstance().getTableBackground()));
		setGridColor(Globals.prefs.getColor(null, JabRefPreferences.getInstance().getGridColor()));
		if (JabRefPreferences.getInstance().isTableShowGrid()) {
			setShowGrid(true);
		} else
		{
			setShowGrid(false);
	...
	public void refreshSorting() {
		...
		if (JabRefPreferences.getInstance().isFloatMarkedEntries()) {
	...
	private void setWidths() {
		...
		String[] widths = Globals.prefs.getStringArray(JabRefPreferences.COLUMN_WIDTHS, null);
	...
	private void setupComparatorChooser() {
		...
		String[] sortFields = new String[] {
				JabRefPreferences.getInstance().getPriSort(),
				JabRefPreferences.getInstance().getSecondary_sort_field(),
				Globals.prefs.getTerSort()
		};
		boolean[] sortDirections = new boolean[] {
				Globals.prefs.isPriDescending(),
				Globals.prefs.isSecDescending(),
				Globals.prefs.isTertiarySortDescending()
		}; // descending
		...
		// Add action listener so we can remember the sort order:
			comparatorChooser.addSortActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent actionEvent) {
					...
					if (count >= 1) {
						Globals.prefs.setPriSort(fields.get(0));
						Globals.prefs.setPriDescending(order.get(0));
					}
					if (count >= 2) {
						Globals.prefs.setSecondray_sort_field(fields.get(1));
						Globals.prefs.setSecDescending(order.get(1));
					}
					else {
						Globals.prefs.setSecondray_sort_field("");
						Globals.prefs.setSecDescending(false);
					}
					if (count >= 3) {
						Globals.prefs.setTerSort(fields.get(2));
						Globals.prefs.setTertiarySortDescending(order.get(2));
					}
					else {
						Globals.prefs.setTerSort("");
						Globals.prefs.setTertiarySortDescending(false);
					}
				}
	...
	public void updateFont() {
		...
		setRowHeight(Globals.prefs.getTableRowPadding() + GUIGlobals.CURRENTFONT.getSize());
	}
	...
	public static void updateRenderers() {
		...
		MainTable.defRenderer = new GeneralRenderer(Globals.prefs.getColor(null, JabRefPreferences.getTableBackground()),
				Globals.prefs.getColor(null, JabRefPreferences.getTableText()));
		Color sel = MainTable.defRenderer.getTableCellRendererComponent
				(new JTable(), "", true, false, 0, 0).getBackground();
		MainTable.reqRenderer = new GeneralRenderer(Globals.prefs.getColor(null, JabRefPreferences.getTableReqFieldBackground()), Globals.prefs.getColor(null, JabRefPreferences.getTableText()));
		MainTable.optRenderer = new GeneralRenderer(Globals.prefs.getColor(null, JabRefPreferences.getInstance().getTableOptFieldBackground()), 
				Globals.prefs.getColor(null, JabRefPreferences.getTableText()));
		MainTable.incRenderer = new IncompleteRenderer();
		MainTable.compRenderer = new CompleteRenderer(Globals.prefs.getColor(null, JabRefPreferences.getTableBackground()));
		MainTable.grayedOutNumberRenderer = new CompleteRenderer(Globals.prefs.getColor(JabRefPreferences.GRAYED_OUT_BACKGROUND, null));
		MainTable.veryGrayedOutNumberRenderer = new CompleteRenderer(Globals.prefs.getColor(JabRefPreferences.VERY_GRAYED_OUT_BACKGROUND, null));
		MainTable.grayedOutRenderer = new GeneralRenderer(Globals.prefs.getColor(JabRefPreferences.GRAYED_OUT_BACKGROUND, null),
				Globals.prefs.getColor(JabRefPreferences.GRAYED_OUT_TEXT, null), MainTable.mixColors(Globals.prefs.getColor(JabRefPreferences.GRAYED_OUT_BACKGROUND, null),
						sel));
		MainTable.veryGrayedOutRenderer = new GeneralRenderer(Globals.prefs.getColor(JabRefPreferences.VERY_GRAYED_OUT_BACKGROUND, null),
				Globals.prefs.getColor(JabRefPreferences.VERY_GRAYED_OUT_TEXT, null), MainTable.mixColors(Globals.prefs.getColor(JabRefPreferences.VERY_GRAYED_OUT_BACKGROUND, null),
						sel));
		...
		for (int i = 0; i < EntryMarker.MARK_COLOR_LEVELS; i++) {
			Color c = Globals.prefs.getColor("markedEntryBackground" + i, null);
			MainTable.markedRenderers[i] = new GeneralRenderer(c,
					Globals.prefs.getColor(null, JabRefPreferences.getTableText()),
					MainTable.mixColors(Globals.prefs.getColor("markedEntryBackground" + i, null), sel));
	...
	static class IncompleteRenderer extends GeneralRenderer {
		...
		public IncompleteRenderer() {
			super(Globals.prefs.getColor(null, JabRefPreferences.getIncompleteEntryBackground()));
