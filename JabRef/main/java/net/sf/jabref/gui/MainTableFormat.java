...
public class MainTableFormat implements TableFormat<BibtexEntry> {
    ...
    public String getColumnName(int col) {
        if (col == 0) {
            ...
            if (Globals.prefs.isShowOneLetterHeadingForIconColumns()) {
    ...
    public void updateTableFormat() {
        ...
        String[] colSettings = Globals.prefs.getStringArray(JabRefPreferences.COLUMN_NAMES, null);
        ...
        namesNatbib = Globals.prefs.isNamesNatbib(); //MK:
        namesLastOnly = JabRefPreferences.getInstance().isNamesLastOnly();
        namesAsIs = JabRefPreferences.getInstance().isNamesAsIs();
        abbr_names = Globals.prefs.isAbbrAuthorNames(); //MK:
        namesFf = Globals.prefs.isNamesFf();
        ...
        if (Globals.prefs.isFileColumn()) {
            iconCols.put(coln, MainTableFormat.FILE);
            coln++;
        }
        if (Globals.prefs.isPdfColumn()) {
            iconCols.put(coln, MainTableFormat.PDF);
            coln++;
        }
        if (Globals.prefs.isUrlColumn()) {
            if (Globals.prefs.isPreferUrlDoi()) {
        ...
        if (Globals.prefs.isArxivColumn()) {
            iconCols.put(coln, MainTableFormat.ARXIV);
            coln++;
        }

        if (Globals.prefs.isExtraFileColumns()) {
            String[] desiredColumns = Globals.prefs.getStringArray(null, JabRefPreferences.getListOfFileColumns());
