...
public class EntryEditorPrefsTab extends JPanel implements PrefsTab {
    ...
    public EntryEditorPrefsTab(JabRefFrame frame, JabRefPreferences prefs) {
        ...
        shortestToComplete = new JSpinner(new SpinnerNumberModel(prefs.getShortestToComplete(), 1, 5, 1));
    ...
    public void setValues() {
        autoOpenForm.setSelected(_prefs.isAutoOpenForm());
        defSource.setSelected(_prefs.getBoolean(JabRefPreferences.DEFAULT_SHOW_SOURCE));
        showSource.setSelected(_prefs.isShowSource());
        ...
        disableOnMultiple.setSelected(_prefs.isDisableOnMultipleSelection());
        autoComplete.setSelected(_prefs.isAutoComplete());
        autoCompFields.setText(_prefs.get(JabRefPreferences.getAutoCompleteFields()));
        shortestToComplete.setValue(_prefs.getShortestToComplete());

        if (_prefs.isAutoCompFF()) {
            autoCompFF.setSelected(true);
        } else if (JabRefPreferences.getInstance().isAutoCompLF()) {
        ...
        if (_prefs.getAutoCompFirstNameMode().equals(JabRefPreferences.AUTOCOMPLETE_FIRSTNAME_MODE_ONLY_ABBR)) {
            autoCompFirstNameMode_Abbr.setSelected(true);
        } else if (_prefs.getAutoCompFirstNameMode().equals(JabRefPreferences.AUTOCOMPLETE_FIRSTNAME_MODE_ONLY_FULL)) {
    ...
    public void storeSettings() {
        _prefs.setAutoOpenForm(autoOpenForm.isSelected());
        ...
        JabRefPreferences.getInstance().setDisableOnMultipleSelection(disableOnMultiple.isSelected());
        // We want to know if the following settings have been modified:
        boolean oldAutoComplete = JabRefPreferences.getInstance().isAutoComplete();
        boolean oldShowSource = _prefs.isShowSource();
        String oldAutoCompFields = _prefs.get(JabRefPreferences.getAutoCompleteFields());
        _prefs.setShortestToComplete((Integer) shortestToComplete.getValue());
        JabRefPreferences.getInstance().setAutoComplete(autoComplete.isSelected());
        _prefs.setAutoCompleteFields(autoCompFields.getText());
        _prefs.setShowSource(showSource.isSelected());
        if (autoCompBoth.isSelected()) {
            _prefs.setAutoCompFF(false);
            JabRefPreferences.getInstance().setAutoCompLF(false);
        }
        else if (autoCompFF.isSelected()) {
            _prefs.setAutoCompFF(true);
            JabRefPreferences.getInstance().setAutoCompLF(false);
        }
        else {
            _prefs.setAutoCompFF(false);
        ...
        if (autoCompFirstNameMode_Abbr.isSelected()) {
            _prefs.setAutoCompFirstNameMode(JabRefPreferences.AUTOCOMPLETE_FIRSTNAME_MODE_ONLY_ABBR);
        } else if (autoCompFirstNameMode_Full.isSelected()) {
            _prefs.setAutoCompFirstNameMode(JabRefPreferences.AUTOCOMPLETE_FIRSTNAME_MODE_ONLY_FULL);
        } else {
            _prefs.setAutoCompFirstNameMode(JabRefPreferences.AUTOCOMPLETE_FIRSTNAME_MODE_BOTH);
        }
