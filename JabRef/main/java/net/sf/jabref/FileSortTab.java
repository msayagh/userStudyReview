...
public class FileSortTab extends JPanel implements PrefsTab {
    ...
    public void setValues() {
        if (_prefs.isSaveInOriginalOrder()) {
            saveInOriginalOrder.setSelected(true);
        } else if (_prefs.isSaveInSpecifiedOrder()) {
        ...
        {
            boolean selected = _prefs.isSaveInSpecifiedOrder();
            savePriSort.setEnabled(selected);
            savePriField.setEnabled(selected);
            savePriDesc.setEnabled(selected);
            saveSecSort.setEnabled(selected);
        ...
        savePriField.setText(JabRefPreferences.getInstance().getSavePriSort());
        saveSecField.setText(_prefs.getSaveSecSort());
        saveTerField.setText(JabRefPreferences.getInstance().getSaveTerSort());

        savePriSort.setSelectedIndex(0);
        saveSecSort.setSelectedIndex(0);
        saveTerSort.setSelectedIndex(0);

        savePriDesc.setSelected(_prefs.isSavePriDescending());
        saveSecDesc.setSelected(JabRefPreferences.getInstance().isSaveSecDescending());
        saveTerDesc.setSelected(_prefs.isSaveTerDescending());

        if (JabRefPreferences.getInstance().isExportInOriginalOrder()) {
            exportInOriginalOrder.setSelected(true);
        } else if (JabRefPreferences.getInstance().isExportInSpecifiedOrder()) {
        ...
        {
            boolean selected = JabRefPreferences.getInstance().isExportInSpecifiedOrder();
            exportPriSort.setEnabled(selected);
            exportPriField.setEnabled(selected);
            exportPriDesc.setEnabled(selected);
            exportSecSort.setEnabled(selected);
        ...
        exportPriField.setText(_prefs.getExportPriSort());
        exportSecField.setText(_prefs.getExportSecSort());
        exportTerField.setText(_prefs.getExportTerSort());

        exportPriSort.setSelectedIndex(0);
        exportSecSort.setSelectedIndex(0);
        exportTerSort.setSelectedIndex(0);

        exportPriDesc.setSelected(_prefs.isExportPriDescending());
        exportSecDesc.setSelected(_prefs.isExportSecDescending());
        exportTerDesc.setSelected(_prefs.isExportTerDescending());
    }

    @Override
    public void storeSettings() {
        _prefs.setSaveInOriginalOrder(saveInOriginalOrder.isSelected());
        _prefs.setSaveInSpecifiedOrder(saveInSpecifiedOrder.isSelected());

        _prefs.setSavePriDescending(savePriDesc.isSelected());
        JabRefPreferences.getInstance().setSaveSecDescending(saveSecDesc.isSelected());
        _prefs.setSaveTerDescending(saveTerDesc.isSelected());

        JabRefPreferences.getInstance().setSavePriSort(savePriField.getText().toLowerCase().trim());
        _prefs.setSaveSecSort(saveSecField.getText().toLowerCase().trim());
        JabRefPreferences.getInstance().setSaveTerSort(saveTerField.getText().toLowerCase().trim());

        JabRefPreferences.getInstance().setExportInOriginalOrder(exportInOriginalOrder.isSelected());
        //_prefs.putBoolean(JabRefPreferences.EXPORT_IN_SPECIFIED_ORDER, exportInSpecifiedOrder.isSelected());
        JabRefPreferences.getInstance().setExportInSpecifiedOrder(exportInSpecifiedOrder.isSelected());

        _prefs.setExportPriDescending(exportPriDesc.isSelected());
        _prefs.setExportSecDescending(exportSecDesc.isSelected());
        _prefs.setExportTerDescending(exportTerDesc.isSelected());

        _prefs.setExportPriSort(exportPriField.getText().toLowerCase().trim());
        _prefs.setExportSecSort(exportSecField.getText().toLowerCase().trim());
        _prefs.setExportTerSort(exportTerField.getText().toLowerCase().trim());
