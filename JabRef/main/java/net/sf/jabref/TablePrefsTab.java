...
class TablePrefsTab extends JPanel implements PrefsTab {
	...
	public void setValues() {
		autoResizeMode
		.setSelected((_prefs.getAutoResizeMode() == JTable.AUTO_RESIZE_ALL_COLUMNS));

		priField.setText(JabRefPreferences.getInstance().getPriSort());
		secField.setText(JabRefPreferences.getInstance().getSecondary_sort_field());
		terField.setText(_prefs.getTerSort());
		priSort.setSelectedIndex(0);
		secSort.setSelectedIndex(0);
		terSort.setSelectedIndex(0);

		if (JabRefPreferences.getInstance().isNamesAsIs()) {
			namesAsIs.setSelected(true);
		} else if (_prefs.isNamesFf()) {
			namesFf.setSelected(true);
		} else if (_prefs.isNamesNatbib()) {
			namesNatbib.setSelected(true);
		} else {
			namesFl.setSelected(true);
		}
		if (_prefs.isAbbrAuthorNames()) {
			abbrNames.setSelected(true);
		} else if (JabRefPreferences.getInstance().isNamesLastOnly()) {
			lastNamesOnly.setSelected(true);
		} else {
			noAbbrNames.setSelected(true);
		}
		priDesc.setSelected(_prefs.isPriDescending());
		secDesc.setSelected(_prefs.isSecDescending());
		terDesc.setSelected(_prefs.isTertiarySortDescending());

		floatMarked.setSelected(JabRefPreferences.getInstance().isFloatMarkedEntries());
	...
	public void storeSettings() {
		...
		JabRefPreferences.getInstance().setNamesAsIs(namesAsIs.isSelected());
		_prefs.setNamesFf(namesFf.isSelected());
		_prefs.setNamesNatbib(namesNatbib.isSelected());
		JabRefPreferences.getInstance().setNamesLastOnly(lastNamesOnly.isSelected());
		_prefs.setAbbrAuthorNames(abbrNames.isSelected());

		_prefs.setAutoResizeMode(
				autoResizeMode.isSelected() ? JTable.AUTO_RESIZE_ALL_COLUMNS : JTable.AUTO_RESIZE_OFF);
		_prefs.setPriDescending(priDesc.isSelected());
		_prefs.setSecDescending(secDesc.isSelected());
		_prefs.setTertiarySortDescending(terDesc.isSelected());
		...
		JabRefPreferences.getInstance().setPriSort(priField.getText().toLowerCase().trim());
		// _prefs.put(JabRefPreferences.SECONDARY_SORT_FIELD, secField.getText().toLowerCase().trim());
		_prefs.setTerSort(terField.getText().toLowerCase().trim());

		JabRefPreferences.getInstance().setFloatMarkedEntries(floatMarked.isSelected());
		// updatefont
