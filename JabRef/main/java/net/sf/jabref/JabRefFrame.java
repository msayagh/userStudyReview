...
public class JabRefFrame extends JFrame implements OutputPrinter {
    ...
    private void init() {
        ...
        UIManager.put("FileChooser.readOnly", Globals.prefs.isFilechooserDisableRename());
        ...
        tlb.setVisible(Globals.prefs.isToolbarVisible());

        setBounds(GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds());
        if ( ! JabRefPreferences.getInstance().isWindowMaximised()) {

           // int sizeX = prefs.getInt(JabRefPreferences.SIZE_X);
        	int sizeX = JabRefPreferences.getInstance().getSize_x();
            int sizeY = JabRefPreferences.getInstance().getSize_y();
            int posX = prefs.getPosX();
            int posY = prefs.getPosY();
    ...
    private void initSidePane() {
        ...
        if (Globals.prefs.isSearchPanelVisible()) {
    ...
    private void tearDownJabRef(Vector<String> filenames) {
        ...
        prefs.setPosX(JabRefFrame.this.getLocation().x);
        prefs.setPosY(JabRefFrame.this.getLocation().y);
  ...
  //      prefs.putInt(JabRefPreferences.SIZE_Y, JabRefFrame.this.getSize().height);
        ...
        JabRefPreferences.getInstance().setWindowMaximised(getExtendedState() == Frame.MAXIMIZED_BOTH);

        prefs.setToolbarVisible(tlb.isVisible());
        prefs.setSearchPanelVisible(sidePaneManager.isComponentVisible("search"));
    ...
    class NewDatabaseAction
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            addTab(database, null, new MetaData(), Globals.prefs.getDefaultEncoding(), true);
    ...
    private void addImportedEntries(final BasePanel panel, final List<BibtexEntry> entries,
         ...
         */
        if (Globals.prefs.isUseImportInspectionDialog() &&
                (Globals.prefs.isUseImportInspectionDialogForSingle() || (entries.size() > 1))) {
            SwingUtilities.invokeLater(new Runnable() {
    ...
    private int addBibEntries(List<BibtexEntry> bibentries, String filename,
        ...
        Util.setAutomaticFields(bibentries, Globals.prefs.isOverwriteOwner(),
                Globals.prefs.isOverwriteTimeStamp(), Globals.prefs.isMarkImportedEntries());
        ...
        if (intoNew || (tabbedPane.getTabCount() == 0)) {
            ...
            BasePanel bp = new BasePanel(JabRefFrame.this, database, null, new MetaData(), Globals.prefs.getDefaultEncoding());
            /*
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            JabRefExecutorService.INSTANCE.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    String[] names = prefs.getStringArray("savedSession", null);
    ...
    class IncreaseTableFontSizeAction extends MnemonicAwareAction {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            GUIGlobals.CURRENTFONT = new Font(GUIGlobals.CURRENTFONT.getFamily(), GUIGlobals.CURRENTFONT.getStyle(),
                    currentSize + 1);
            Globals.prefs.setFontSize(currentSize + 1);
