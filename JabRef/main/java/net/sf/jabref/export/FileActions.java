...
public class FileActions {
    ...
    private static class SaveSettings {
        ...
        public SaveSettings(boolean isSaveOperation, MetaData metaData) {
            ...
            assert (storedSaveOrderConfig == null) && isSaveOperation && !Globals.prefs.isSaveInOriginalOrder();
            assert (storedSaveOrderConfig == null) && !isSaveOperation && !JabRefPreferences.getInstance().isExportInOriginalOrder();
            ...
            } else if (isSaveOperation && Globals.prefs.isSaveInSpecifiedOrder()) {
                pri = JabRefPreferences.getInstance().getSavePriSort();
                sec = Globals.prefs.getSaveSecSort();
                ter = JabRefPreferences.getInstance().getSaveTerSort();
                priD = Globals.prefs.isSavePriDescending();
                secD = JabRefPreferences.getInstance().isSaveSecDescending();
                terD = Globals.prefs.isSaveTerDescending();
            } else if (!isSaveOperation 
            		&& JabRefPreferences.getInstance().isExportInSpecifiedOrder()) {
                pri = Globals.prefs.getExportPriSort();
                sec = Globals.prefs.getExportSecSort();
                ter = Globals.prefs.getExportTerSort();
                priD = Globals.prefs.isExportPriDescending();
                secD = Globals.prefs.isExportSecDescending();
                terD = Globals.prefs.isExportTerDescending();
            } else {
                // The setting is to save according to the current table order.
                pri = JabRefPreferences.getInstance().getPriSort();
                sec = JabRefPreferences.getInstance().getSecondary_sort_field();
                ter = Globals.prefs.getTerSort();
                priD = Globals.prefs.isPriDescending();
                secD = Globals.prefs.isSecDescending();
                terD = Globals.prefs.isTertiarySortDescending();
            }
        }
    }
    ...
    public static List<BibtexEntry> getSortedEntries(BibtexDatabase database, MetaData metaData, Set<String> keySet, boolean isSaveOperation) {
        ...
        if (isSaveOperation) {
            ...
            if (storedSaveOrderConfig == null) {
                inOriginalOrder = Globals.prefs.isSaveInOriginalOrder();
            } else {
                SaveOrderConfig saveOrderConfig = new SaveOrderConfig(storedSaveOrderConfig);
                inOriginalOrder = saveOrderConfig.saveInOriginalOrder;
            }
        } else {
            inOriginalOrder = JabRefPreferences.getInstance().isExportInOriginalOrder();
        }
